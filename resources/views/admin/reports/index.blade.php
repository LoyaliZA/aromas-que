            ...
            <div class="flex-1 bg-aromas-secondary/50 border border-aromas-tertiary/30 rounded-xl p-6 shadow-inner relative overflow-hidden">
                ...
                <div x-show="activeTab === 'general'" style="display: none;">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        <div class="bg-gray-800/50 p-5 rounded-xl border border-gray-700"><p class="text-xs text-gray-400 uppercase font-bold text-center">Atendidos</p><p class="text-4xl font-black text-green-400 text-center">{{ $metrics['total_served'] }}</p></div>
                        <div class="bg-gray-800/50 p-5 rounded-xl border border-gray-700"><p class="text-xs text-gray-400 uppercase font-bold text-center">Prom. Espera</p><p class="text-3xl font-black text-yellow-400 text-center">{{ $metrics['formatted_wait_time'] }}</p></div>
                        <div class="bg-gray-800/50 p-5 rounded-xl border border-gray-700"><p class="text-xs text-gray-400 uppercase font-bold text-center">Prom. Atención</p><p class="text-3xl font-black text-blue-400 text-center">{{ $metrics['formatted_service_time'] }}</p></div>
                        <div class="bg-gray-800/50 p-5 rounded-xl border border-gray-700"><p class="text-xs text-gray-400 uppercase font-bold text-center">Abandonos</p><p class="text-4xl font-black text-red-500 text-center">{{ $metrics['total_abandoned'] }}</p></div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-900 rounded-xl border border-gray-700 shadow-md flex flex-col h-full max-h-[380px]">
                            <div class="p-5 border-b border-gray-800"><h3 class="text-sm font-bold text-gray-300 uppercase tracking-widest">Desempeño Vendedores</h3></div>
                            <div class="overflow-y-auto flex-1 custom-scrollbar">
                                <table class="w-full text-left border-collapse">
                                    <thead class="bg-gray-800/50 sticky top-0 z-10">
                                        <tr class="text-gray-400 text-[10px] uppercase tracking-wider"><th class="py-3 px-4">Vendedor</th><th class="py-3 px-2 text-center">Ventas</th><th class="py-3 px-2 text-center">Prom. Atn.</th><th class="py-3 px-4 text-right">Pausas</th></tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-800">
                                        @forelse($employees_metrics as $emp)
                                            <tr class="hover:bg-gray-800"><td class="py-3 px-4 text-sm text-white truncate max-w-[120px]">{{ $emp['name'] }}</td><td class="py-3 px-2 text-sm text-green-400 font-bold text-center">{{ $emp['served'] }}</td><td class="py-3 px-2 text-xs text-blue-300 text-center">{{ $emp['formatted_avg_service'] }}</td><td class="py-3 px-4 text-xs text-yellow-500 text-right">{{ $emp['formatted_break_time'] }}</td></tr>
                                        @empty
                                            <tr><td colspan="4" class="py-8 text-center text-sm text-gray-500">No hay datos.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                ...
                <div x-show="activeTab === 'clients'" style="display: none;">
                    ...
                    <div class="bg-gray-900 rounded-xl border border-gray-700 overflow-hidden shadow-md">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-gray-800/50 border-b border-gray-700"><tr class="text-gray-400 text-[10px] uppercase tracking-wider"><th class="p-4">Turno</th><th class="p-4">Cliente</th><th class="p-4">Vendedor</th><th class="p-4 text-center">Espera</th><th class="p-4 text-center">Atención</th><th class="p-4 text-center">Estado</th></tr></thead>
                                <tbody class="divide-y divide-gray-800">
                                    @forelse($detailedClients as $client)
                                        <tr class="hover:bg-gray-800/50">
                                            <td class="p-4 font-mono font-bold text-white">{{ $client->turn_number }}</td><td class="p-4 text-sm text-gray-300">{{ $client->client_name }}</td><td class="p-4 text-sm text-gray-300">{{ $client->assignedShift->employee->full_name ?? 'N/A' }}</td><td class="p-4 text-sm text-yellow-400 font-mono text-center">{{ $client->formatted_wait }}</td><td class="p-4 text-sm text-blue-400 font-mono text-center">{{ $client->formatted_serve }}</td>
                                    ...
                                    @empty <tr><td colspan="6" class="p-8 text-center text-gray-500">No hay registros.</td></tr> @endforelse
                                </tbody>
                            </table>
                        </div>
                ...
                <div x-show="activeTab === 'abandoned'" style="display: none;">
                    ...
                    <div class="bg-gray-900 rounded-xl border border-gray-700 overflow-hidden shadow-md">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-gray-800/50 border-b border-gray-700"><tr class="text-gray-400 text-[10px] uppercase tracking-wider"><th class="p-4">Turno</th><th class="p-4">Cliente</th><th class="p-4">Fecha / Hora</th><th class="p-4">Motivo</th></tr></thead>
                                <tbody class="divide-y divide-gray-800">
                                    @forelse($detailedAbandoned as $abandoned)
                                        <tr class="hover:bg-gray-800/50"><td class="p-4 font-mono font-bold text-white">{{ $abandoned->turn_number }}</td><td class="p-4 text-sm text-gray-300">{{ $abandoned->client_name }}</td><td class="p-4 text-sm text-gray-500">{{ \Carbon\Carbon::parse($abandoned->queued_at)->format('d/m/Y H:i') }}</td><td class="p-4 text-sm text-red-400 italic">@if($abandoned->abandonment_reason_id) {{ $abandoned->abandonmentReason->name }} @elseif($abandoned->custom_abandonment_reason) {{ $abandoned->custom_abandonment_reason }} @else Cancelado por inactividad @endif</td></tr>
                                    @empty <tr><td colspan="4" class="p-8 text-center text-gray-500">No hay abandonos.</td></tr> @endforelse
                                </tbody>
                            </table>
                        </div>
                ...
                <div x-show="activeTab === 'performance'" style="display: none;">
                        ...
                        @if($empData)
                            <div class="flex gap-2">
                                <a href="{{ route('admin.reports.export', ['type' => 'employee', 'employee_id' => $selectedEmployeeId, 'format' => 'csv', 'period' => $period, 'start_date' => $start_date, 'end_date' => $end_date]) }}" class="bg-gray-800 hover:bg-gray-700 text-white font-bold px-4 py-2 rounded-lg text-sm border border-gray-600 transition flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg> CSV
                                </a>
                    ...
                    @if($empData)
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            ...
                            <div class="bg-gray-900 p-5 rounded-xl border border-gray-700 text-center"><p class="text-xs text-gray-400 uppercase font-bold tracking-widest mb-2">Tiempo en Pausa (Total)</p><p class="text-4xl font-black text-yellow-500">{{ $empData['kpis']['total_break'] }}</p></div>
                        </div>
                        ...
                        {{-- Tabla Empleado --}}
                        <div class="bg-gray-900 rounded-xl border border-gray-700 overflow-hidden shadow-md">
                            <div class="p-4 border-b border-gray-700 bg-gray-800/50"><h3 class="text-sm font-bold text-gray-300 uppercase tracking-widest">Historial de Clientes</h3></div>
                            <div class="overflow-x-auto">
                                <table class="w-full text-left border-collapse">
                                    <thead class="bg-gray-800/50 border-b border-gray-700">
                                        <tr class="text-gray-400 text-[10px] uppercase tracking-wider"><th class="p-4">Turno</th><th class="p-4">Cliente</th><th class="p-4 text-center">Tipo</th><th class="p-4 text-center">Espera</th><th class="p-4 text-center">Atención</th><th class="p-4 text-center">Estado</th></tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-800">
                                        ...
                                        @empty 
                                            <tr><td colspan="6" class="p-8 text-center text-gray-500">Sin clientes atendidos.</td></tr> 
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                    ...
                    @else
                        <div class="text-center py-20 bg-gray-900 rounded-xl border border-gray-700"><p class="text-gray-500">Selecciona un vendedor en la parte superior para ver sus estadísticas.</p></div>
                    @endif
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            if (typeof Chart !== 'undefined') {
                ...
                if (ctxGlobal) {
                    ...
                    new Chart(ctxGlobal, {
                        type: 'line', data: { labels: chartData.labels, datasets: [{ label: 'Turnos Atendidos', data: chartData.data, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', fill: true, tension: 0.4, borderWidth: 3, pointRadius: 4, pointBackgroundColor: '#FDC974' }] },
        ...
        function reportsDashboard() {
            return {
                ...
                initDashboard() { if (this.activeTab === 'realtime') { this.startRealTime(); } },
                switchTab(tab) {
                    this.activeTab = tab;
                ...
                stopRealTime() { clearInterval(this.pollingInterval); clearInterval(this.clockInterval); },
                fetchRealTimeData() {
