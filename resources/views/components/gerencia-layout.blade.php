...
<head>
    ...
    <title>Panel de Gerencia - Aromas</title>
...
<body class="bg-aromas-main text-gray-200 font-sans antialiased" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="mt-5 px-4 space-y-2">
            ...
            @if(Auth::user()->isAdmin() || Auth::user()->canManageShifts() || Auth::user()->canManageSellers())
            <a href="{{ route('gerencia.staff.index') }}" 
               class="flex items-center px-4 py-3 rounded-lg transition-all group {{ request()->routeIs('gerencia.staff.*') ? 'bg-aromas-highlight text-aromas-main font-bold shadow-lg shadow-aromas-highlight/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('gerencia.staff.*') ? 'text-aromas-main' : 'text-gray-500 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                Personal y Turnos
            </a>
            @endif
