<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-gray-700 shadow-xl flex flex-col overflow-hidden group hover:border-aromas-highlight/50 transition-colors">
        ...
        {{-- FOOTER / ACCIÓN --}}
        <div class="p-4 pt-0">
            @if($pickup->currentStatus?->code === 'PRE_REGISTERED' || $pickup->currentStatus?->code === 'NEEDS_CORRECTION')
            {{-- BOTÓN FASE 2: COMPLETAR REGISTRO --}}
            @if($pickup->currentStatus?->code === 'NEEDS_CORRECTION')
            <div class="mb-3 bg-red-500/10 border border-red-500/30 p-2 rounded-lg text-xs text-red-400">
                <strong class="block uppercase tracking-wider mb-1">⚠️ Corrección solicitada:</strong>
                {{ $pickup->correction_notes }}
            </div>
            @endif
            <button @click="$dispatch('open-complete-modal', { 
            id: {{ $pickup->id }}, 
            ...
            initial_evidence: '{{ $pickup->initial_evidence_path ? asset('storage/'.$pickup->initial_evidence_path) : '' }}'
                ...
                class="w-full py-4 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-black tracking-widest uppercase flex items-center justify-center gap-3 transition-all shadow-lg active:scale-95">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- BOTÓN: ENTREGAR AL CLIENTE (Ya aprobado por gerente) --}}
            ...
            @elseif($pickup->currentStatus?->code === 'PENDING_CONFIRMATION')
            <div class="w-full bg-amber-500/10 text-amber-500 font-bold py-4 rounded-xl text-center border border-amber-500/20 text-sm tracking-widest uppercase flex items-center justify-center gap-2">
